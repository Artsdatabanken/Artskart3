...
export class MapService {
  ...
  activeSiteChangedAction$ = this.activeSite.asObservable();
    ...
    getData(){
       ...
       // this.map.addPoint(layerId: string, coord: MapCoord, properties?: Record<string, unknown>, style?: DrawStyleOptions, opts?: InsertGeomOptions): boolean { return this.engine.addPoint(layerId, coord, properties, style, opts); }
    }
    ...
    useTestData =():any=>{
      ...
      /** Central helper: remove + add geojson vector layer */
  replaceGeoJsonVectorLayer(
    map: NbicMapComponent,
    args: GeoJsonVectorLayerArgs
  ) {
    map.removeLayer(args.id);
    map.addLayer({
      id: args.id,
      kind: 'vector',
      visible: args.visible ?? true,
      source: this.geojsonSource(JSON.stringify(args.fc)),
      style: args.style,
      cluster: args.cluster,
      hover: args.hover,
      zIndex: args.zIndex,
      zIndexPinned: args.zIndexPinned
    } as any);
  }

  private geojsonSource(text: string) {
    return {
      type: 'geojson' as const,
      options: {
        dataProjection: this.DATA_PROJ,
        featureProjection: this.VIEW_PROJ,
        text
      }
    };
  }
